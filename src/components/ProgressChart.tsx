...
export const ProgressChart = ({ history, unit, theme }: ProgressChartProps) => {
            ...
            if (entry.estimatedOneRepMaxes) {
                lifts.forEach(lift => {
                    ...
                    const val = entry.estimatedOneRepMaxes![lift as keyof typeof entry.estimatedOneRepMaxes] ||
                        entry.estimatedOneRepMaxes![lift.charAt(0).toUpperCase() + lift.slice(1) as keyof typeof entry.estimatedOneRepMaxes];
    ...
    if (sortedData.length === 0) {
        return (
            <View style={[styles.chartContainer, { backgroundColor: theme.colors.card, justifyContent: 'center', alignItems: 'center' }]}>
                <Text style={{ color: theme.colors.subtext }}>Not enough data for chart</Text>
            </View>
        );
    }
